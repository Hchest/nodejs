...
胰体尾：胰体尾切除术（保脾）、胰体尾脾切除术、根治性顺行模块胰体尾脾切除术、胰体尾脾肿物切除术（腹腔镜）

胰头颈：胰十二指肠切除术、胰十二指肠切除术（保留幽门）、胰十二指肠切除术（联合门静脉壁切除）、保留十二指肠的胰头肿物切除术

```sql
-- 10363 中有肿瘤位置的为 10085
SELECT PATIENT_NO FROM [dbo].[PAT_VISIT] WHERE SD_CODE='YXA_O' AND SD_GROUP='1' AND DISCHARGE_DATE>='2016-01-01 00:00:00.000' AND DISCHARGE_DATE<='2018-12-31 00:00:00.000'
...
-- 慢性胰腺炎 YXA_O_277
-- 胰管结石 YXA_O_279
SELECT distinct PATIENT_NO FROM [dbo].[PAT_FOLLOW_UP_RESULT] WHERE SD_ITEM_CODE='YXA_O_259' AND SD_ITEM_VALUE='1' AND PATIENT_NO  IN (
	SELECT PATIENT_NO FROM [dbo].[PAT_VISIT] WHERE SD_CODE='YXA_O' AND SD_GROUP='1' AND DISCHARGE_DATE>='2016-01-01 00:00:00.000' AND DISCHARGE_DATE<='2018-12-31 00:00:00.000' 
)
...
SELECT 
	PATIENT_NO, 
	MIN ( SD_ITEM_VALUE ) AS m 
FROM [dbo].[PAT_FOLLOW_UP_RESULT]
WHERE 
SD_ITEM_CODE='YXA_O_250'
AND PATIENT_NO  IN (
		SELECT PATIENT_NO FROM [dbo].[PAT_VISIT] WHERE SD_CODE='YXA_O' AND SD_GROUP='1' AND DISCHARGE_DATE>='2016-01-01 00:00:00.000' AND DISCHARGE_DATE<='2018-12-31 00:00:00.000' 
...
根治术减三例
	一个是生存61个月的
    另外两个是手术方式使用 纳米刀辅助，手术名称 胰十二指肠切除术的两个患者。
    人数是 8597-3=8594

数据定义如下：	

OS： 死亡的患者 死亡时间-手术时间，未死亡的患者 末次随访时间-手术时间，死亡状态未知 不参与计算
DFS： 复发转移的患者 复发转移时间-手术时间，未复发转移的患者 分两组 有死亡日期的 死亡时间-手术时间 没有死亡时间 末次随访时间-手术时间，复发转移状态未知 不参与计算

年龄 ：<65 为1， >=65为2
血管受累：是 1 ，否 2	
肿瘤大小分组： 小于等于2cm 为1，大于2cm 为2
肿瘤分化：低 1，中 2，高 3	
	分为高、中、低；有合并项则原则为往低划分，比如高中划为中，中低分为低，未分化和低分化算一起
TNM分期（第七版）：IA IB IIA IIB III 为早期 1，IV为晚期 2	
BMI：正常1，超重2，肥胖3。缺失1219例(考虑是否纳入，医生确定不纳入)	
	正常（18.5-24.9kg/m2）、超重（25-29.9 kg/m2）、肥胖（大于30 kg/m2），小于18.5的算正常
淋巴结转移：是 1 ，否 2。缺失7909例(影响总样本量，未纳入)	
...
1. 胰腺导管腺癌
2. 手术方式排出(13.探查术 14.活检术 15.胰腺肿物剜除术)
...
WHERE
	SD_ITEM_CODE = 'YXA_O_151' 
	AND SD_ITEM_VALUE NOT IN ( '13', '14', '15' ) 
	AND PATIENT_NO IN ( SELECT PATIENT_NO FROM [dbo].[PAT_VISIT] WHERE SD_CODE = 'YXA_O' AND SD_GROUP = '1' )
```
